# Capability: CAP-012 Agent Assisted Modeling

Support agent-assisted modeling workflows (chat, scoped context, proposals, and validation-gated edits) as a first-class authoring mode.



## Attributes

_No attributes defined._

## References

_No references defined._

## Links

- requires [PRO-012](../Process/PRO-012-Agent_Assisted_Modeling.md)
- requires [PRO-013](../Process/PRO-013-Serve_Model_Tool_Surface_MCP.md)


## Version

## Audit Log

| Timestamp | Editor | Change |
|-----------|--------|--------|
| 2026-02-21T00:00:00Z | Architect | create |
| 2026-02-21T18:00:00Z | Architect | change |
