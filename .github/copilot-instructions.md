...
Create and maintain a `PROGRESS.md` file in the root of the repository that contains the complete implementation plan, and the current status of implementation. The progress tracker MUST be kept up to date at all times. The `PROGRESS.md` file is only meant for agent use and is not intended for end users.
